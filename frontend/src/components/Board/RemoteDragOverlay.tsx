      ...
      {type === 'column' ? (
        ...
        >
          ...
          <div className="p-3 pt-4 font-semibold text-slate-800 flex items-center">
            ...
            <span className="ml-2 bg-slate-200 text-slate-600 px-2 py-0.5 rounded-full text-xs">{(item as any).tasks?.length || 0}</span>
          </div>
          
          <div className="flex-1 p-3 pt-0 flex flex-col gap-3 overflow-hidden pointer-events-none">
            {(item as any).tasks?.slice(0, 5).map((task: any) => (
              <div key={task.id} className="bg-white p-3 rounded-lg shadow-sm border border-slate-200 pointer-events-none">
                <span className="text-sm font-medium text-slate-700">{task.title}</span>
              </div>
            ))}
            {((item as any).tasks?.length || 0) > 5 && (
              <div className="text-center text-xs text-slate-400 font-medium py-1">
                + {((item as any).tasks?.length || 0) - 5} more tasks
              </div>
            )}
          </div>

          <div className="absolute -top-3 -right-2 bg-blue-500 text-white text-xs px-2.5 py-1 rounded-full shadow-md truncate max-w-[150px] font-medium tracking-wide z-10 border-2 border-white pointer-events-none">
            {user?.name || user?.email || 'Someone is moving this'}
          </div>
        </div>
      ) : (
        <div 
          className="bg-white p-3 rounded-lg shadow-[0_4px_12px_rgba(59,130,246,0.15)] border-2 border-blue-400 opacity-100 break-words flex items-center relative h-full w-full"
          style={{ animation: 'pulseGlow 2s cubic-bezier(0.4, 0, 0.6, 1) infinite' }}
        >
          ...
          <div className="absolute -top-8 right-0 bg-blue-500 text-white text-xs px-2.5 py-1 rounded-full shadow-md truncate max-w-[150px] font-medium tracking-wide pointer-events-none">
            {user?.name || user?.email || 'Someone is moving this'}
          </div>
        </div>
      )}
